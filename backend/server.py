...
def _is_meaningful_record(record: dict) -> bool:
    return any([
        ...
        float(record.get("costo_t", 0) or 0) != 0, # Cambiado de > a !=
        float(record.get("costo_l", 0) or 0) != 0,  # Cambiado de > a !=
...
async def create_record(data: RecordCreate):
    """Create a new record con nueva lógica de totales"""
    # TOTAL = Costo L
    # Saldo a Favor = Costo L - Costo T
    total = data.costo_l
...
async def update_record(record_id: str, data: RecordUpdate):
    """Update con recalculación según nuevas reglas"""
    async with _records_lock:
        records = await load_records()
        ...
        # Recalcular valores basados en la nueva lógica
...
async def get_totals():
    """Get totals including the new Saldo a Favor window"""
    async with _records_lock:
        records = await load_records()
    ...
    # Suma global de todos los saldos a favor
            ...
            for sheet in workbook.worksheets:
                ...
                for row_num, row in enumerate(sheet.iter_rows(min_row=header_row + 1, values_only=True), start=header_row + 1):
                    try:
                        ...
                        # REGLA DE ORO: Total es el costo local. Saldo es la resta.
                        total = costo_l
