...
const PremiumDashboardTransportista = ({ records = [] }) => {
  ...
  const clientsData = useMemo(() => {
    ...
    records.forEach(record => {
      ...
      const total = parseFloat(record.total || record.costo_l || 0);
      const saldoFavor = parseFloat(record.saldo_a_favor || 0);

      data.totalSaldoFavor += saldoFavor;
  ...
  const topTransportistas = useMemo(() => {
    if (!selectedClientData) return [];
    return Object.entries(selectedClientData.transportistas)
      .map(([nombre, valor]) => ({ nombre, valor }))
      .sort((a, b) => b.valor - a.valor)
      .slice(0, 5);
  }, [selectedClientData]);
  ...
  const mayorVenta = useMemo(() => {
    if (!selectedClientData) return 0;
      ...
      {selectedClientData && (
        <div className="space-y-6">
          <div className="bg-gradient-to-r from-[#002FA7] to-[#0052CC] text-white p-6 rounded-sm">
            ...
            <p className="text-sm opacity-90">Análisis detallado del cliente - Transporte</p>
          </div>
...
export default PremiumDashboardTransportista;
