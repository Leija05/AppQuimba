...
const DeveloperSignature = ({ position = "bottom-right" }) => {
  ...
  return (
    <div
      className={`fixed ${positionClasses[position]} z-30 pointer-events-none select-none`}
      aria-hidden="true"
      style={{ userSelect: "none", WebkitUserSelect: "none", opacity: 0.45 }}
    >
      <svg
      ...
      >
        <g transform="translate(0,672) scale(0.1,-0.1)" fill="none" stroke="currentColor">
          {SIGNATURE_PATHS.map((d, idx) => (
            <path
              key={`signature-path-${idx}`}
              d={d}
              pathLength="1"
              className="signature-path"
              style={{
                animationDelay: `${idx * 1.2}s`,
                animationDuration: `${8 + idx * 0.8}s`,
              }}
            />
          ))}
        </g>
      ...
      <style jsx>{`
        .signature-svg {
          ...
          filter: drop-shadow(0 1px 1px rgba(2, 6, 23, 0.08));
        }
        .signature-path {
          stroke-width: 30;
